...
import { handleCsvAnalyticsDiagnosticsApiRoute } from './csv-analytics-diagnostics-api.js';
...
const CSV_ANALYTICS_ROUTE_PATTERN = /^\/api\/v1\/stores\/[^/]+\/csv-analytics\/(overview|daily|campaign|ad-group|ad_group|targeting|search-term|search_term|match-type|match_type|quality|diagnostics)$/;
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (CSV_ANALYTICS_ROUTE_PATTERN.test(url.pathname)) {
        const diagnosticsResponse = await handleCsvAnalyticsDiagnosticsApiRoute({ request, env, actor, url });
