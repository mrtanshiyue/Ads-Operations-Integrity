...
export async function handleCsvImportsApiRoute({ request, env, actor, url }) {
  ...
  if (resource === 'search-terms' && !child) {
    ...
    return uploadSearchTerms(request, env.CONTROL_DB, route, actor, storeId, url);
  }
...
async function uploadSearchTerms(request, controlDb, route, actor, storeId, url) {
  ...
  const action = outcome.action === 'csv_import_duplicate'
    ...
    : outcome.published
      ? 'csv_import.published'
      : 'csv_import.rejected';
  await audit(controlDb, request, actor.user_id, storeId, action, 'csv_import', outcome.importId, {
    sourceFileName,
...
async function listImports(request, route, url) {
  ...
  return json(request, {
    ...
    items,
    nextBefore: hasMore ? items.at(-1)?.uploadedAt || null : null,
  }, 200);
}
...
async function importDetail(request, route, importId) {
  ...
  const fact = await route.storeDb.prepare(`
