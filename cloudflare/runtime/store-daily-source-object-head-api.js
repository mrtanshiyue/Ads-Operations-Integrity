...
export async function handleStoreDailySourceObjectHeadApiRoute({ request, env, actor, url }) {
  ...
  const enrichedItems = items.map((item, index) => {
    ...
    const evidence = sourceR2ObjectHeadIdentity(
      context.sourceContent,
      context.sourceObject,
      observations.get(context.sourceObject.r2ObjectKey),
    );
    return {
      ...item,
      sourceR2ObjectHeadObserved: evidence.observed,
      sourceR2ObjectExists: evidence.exists,
      sourceR2ObjectHeadIdentityValid: evidence.valid,
    };
  });
