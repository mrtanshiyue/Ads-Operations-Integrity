...
const receipt = {
  schemaVersion: 'cloudflare-human-review-acceptance-permission-probe-v1',
  runId: RUN_ID,
...
try {
  const accessApps = await probe('accessAppsRead', `/access/apps?domain=${encodeURIComponent(PROD_ACCESS_DOMAIN)}&exact=true`);
  const targetApp = Array.isArray(accessApps.result)
    ? accessApps.result.find((app) => String(app?.domain || '').toLowerCase() === PROD_ACCESS_DOMAIN.toLowerCase())
    : null;
  if (accessApps.status === 200) {
    assert(targetApp?.id, `Production Access app not found for ${PROD_ACCESS_DOMAIN}`);
    receipt.runtime.productionAccessApp = { id: targetApp.id, name: targetApp.name || null, domain: targetApp.domain || null };
    receipt.checks.productionAccessAppResolved = true;
    await probe('accessPoliciesRead', `/access/apps/${encodeURIComponent(targetApp.id)}/policies`);
  }
  ...
  if (workers.status === 200) {
    assert(dev?.id && prod?.id && sync?.id, 'Expected Dev/Production/Sync Workers were not all visible');
  ...
  if (prod?.tag) {
    const latestBuild = await probe('workersBuildMetadataRead', `/builds/builds/latest?external_script_ids=${encodeURIComponent(prod.tag)}`);
    receipt.runtime.productionLatestBuildVisible = latestBuild.status === 200;
  } else {
    receipt.capabilities.workersBuildMetadataRead = { status: null, ok: false, reason: 'production_worker_tag_unavailable' };
  }

  const accessKeys = ['accessAppsRead', 'accessPoliciesRead', 'accessServiceTokensList'];
  const nonAccessKeys = [
    'd1MetadataRead',
    'd1SelectQuery',
    'workersScriptsRead',
  ...
  const accessPass = accessKeys.every((key) => receipt.capabilities[key]?.ok === true);
  ...
  if (accessPass && nonAccessPass) {
    receipt.result = 'PASS';
  } else if (!accessPass && nonAccessPass && accessKeys.some((key) => receipt.capabilities[key]?.status === 403)) {
  ...
  if (receipt.result === 'FAIL') {
    const accessStatuses = ['accessAppsRead', 'accessPoliciesRead', 'accessServiceTokensList']
      .map((key) => receipt.capabilities[key]?.status)
      .filter((value) => value != null);
    if (accessStatuses.includes(403)) receipt.result = 'BLOCKED_BY_CLOUDFLARE_ACCESS_TOKEN_PERMISSION';
  }
} finally {
  receipt.finishedAt = new Date().toISOString();
  await writeFile(`${OUT}/receipt.json`, `${JSON.stringify(receipt, null, 2)}\n`);
  console.log(JSON.stringify({ result: receipt.result, checks: receipt.checks, capabilityStatuses: Object.fromEntries(Object.entries(receipt.capabilities).map(([key, value]) => [key, value.status])) }));
}
...
async function probe(name, path) {
  ...
  receipt.capabilities[name] = {
    ...
    errorCodes: Array.isArray(response.body?.errors) ? response.body.errors.map((error) => error?.code).filter((value) => value != null) : [],
  };
...
async function probeD1Select() {
  ...
  const response = await cf(path, { method: 'POST', body: { sql: 'SELECT 1 AS permission_probe' }, allowReadOnlyD1Query: true });
  const ok = response.status >= 200 && response.status < 300 && response.body?.success !== false;
  receipt.capabilities.d1SelectQuery = {
    status: response.status,
    ok,
    errorCodes: Array.isArray(response.body?.errors) ? response.body.errors.map((error) => error?.code).filter((value) => value != null) : [],
  };
