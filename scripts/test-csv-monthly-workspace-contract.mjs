...
assert.match(uiSource, /it is not net profit/i);
...
assert.match(uiSource, /CSV_MONTHLY_WORKSPACE_SOURCE_RECEIPT_MISMATCH/);
assert.match(uiSource, /CSV_MONTHLY_WORKSPACE_BATCH_COUNT_MISMATCH/);
assert.match(uiSource, /CSV_MONTHLY_WORKSPACE_DUPLICATE_HASH_EVIDENCE/);
...
assert.equal(workspace.source.receiptHashSetVerified, true);
...
assert.equal(workspace.summary.multiSourceMonthCount, 1);
...
assert.throws(
  () => mod.buildCsvMonthlyOperatingWorkspace({ ...result, source: { ...result.source, kind: 'amazon_api' } }),
  (error) => error?.code === 'CSV_MONTHLY_WORKSPACE_SOURCE_KIND_INVALID',
  'Monthly workspace must reject non-CSV source kind',
);
assert.throws(
...
assert.throws(
  () => mod.buildCsvMonthlyOperatingWorkspace({ ...result, source: { ...result.source, contentSha256s: ['e'.repeat(64), ...result.source.contentSha256s.slice(1)] } }),
  (error) => error?.code === 'CSV_MONTHLY_WORKSPACE_SOURCE_RECEIPT_MISMATCH',
  'Monthly workspace must reject source/receipt hash drift',
);
assert.throws(
  () => mod.buildCsvMonthlyOperatingWorkspace({ ...result, source: { ...result.source, batchCount: 4 } }),
  (error) => error?.code === 'CSV_MONTHLY_WORKSPACE_BATCH_COUNT_MISMATCH',
  'Monthly workspace must reject batch-count drift',
);
assert.throws(
  () => mod.buildCsvMonthlyOperatingWorkspace({
    ...result,
    source: { ...result.source, contentSha256s: [result.source.contentSha256s[0], result.source.contentSha256s[0], result.source.contentSha256s[2]] },
    imports: [result.imports[0], { ...result.imports[1], contentSha256: result.imports[0].contentSha256 }, result.imports[2]],
  }),
  (error) => error?.code === 'CSV_MONTHLY_WORKSPACE_DUPLICATE_HASH_EVIDENCE',
  'Monthly workspace must reject duplicate hash evidence',
);
assert.throws(
  () => mod.buildCsvMonthlyOperatingWorkspace({ ...result, periods: { ...result.periods, monthlySnapshots: [{ ...result.periods.monthlySnapshots[0], amazonMutationAuthorized: true }] } }),
  (error) => error?.code === 'CSV_MONTHLY_WORKSPACE_AUTHORITY_ESCALATION_BLOCKED',
  'Monthly workspace must reject snapshot-level Amazon mutation authority',
);
...
console.log(JSON.stringify({
  ...
  monthSelector: true,
  mutuallyExclusiveOperatingStates: true,
  monthlyMetrics: true,
  monthOverMonth: true,
  sourceReceiptMapping: true,
  currencyEvidencePreserved: true,
...
function fixture() {
  ...
  return {
    source: {
      kind: 'csv_import_set',
      batchCount: 3,
      contentSha256s: [janHash, febHash, marHash],
...
function month(key, startDate, endDate, expected, covered, analyticalDecisionUse, reliabilityState, change) {
  return {
    ...
    startDate,
    endDate,
    monthComplete: covered === expected,
