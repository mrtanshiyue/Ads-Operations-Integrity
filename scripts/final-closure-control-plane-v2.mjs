...
export async function collectFinalClosureEvidence(options = {}) {
  ...
  const runtimeToken = requiredText(options.runtimeToken || process.env.CLOUDFLARE_API_TOKEN, 'FINAL_CLOSURE_RUNTIME_TOKEN_REQUIRED');
  ...
  const requiredCheck = (checks?.check_runs || [])
    .filter((check) => check?.name === EXPECTED.requiredContext)
    .sort((a, b) => Date.parse(b.completed_at || 0) - Date.parse(a.completed_at || 0))[0];
  const ci = {
    requiredContext: EXPECTED.requiredContext,
    conclusion: String(requiredCheck?.conclusion || 'missing'),
  };

  const workers = await cfRuntime('GET', '/workers/scripts');
  const workerTagByName = new Map(
    (Array.isArray(workers?.result) ? workers.result : []).map((worker) => [worker.id, worker.tag]),
  );
  const [dev, prod, sync] = await Promise.all([
    collectRuntime({ cf: cfRuntime, workerName: 'ads-operations-web-dev', workerTag: workerTagByName.get('ads-operations-web-dev') }),
    collectRuntime({ cf: cfRuntime, workerName: 'ads-operations-web-prod', workerTag: workerTagByName.get('ads-operations-web-prod') }),
    collectRuntime({ cf: cfRuntime, workerName: 'ads-operations-sync-prod', workerTag: workerTagByName.get('ads-operations-sync-prod'), buildRequired: false }),
  ]);

  const [devVersion, prodVersion, syncVersion, syncSchedules] = await Promise.all([
    cfRuntime('GET', `/workers/scripts/ads-operations-web-dev/versions/${encodeURIComponent(dev.versionId)}`),
    cfRuntime('GET', `/workers/scripts/ads-operations-web-prod/versions/${encodeURIComponent(prod.versionId)}`),
    cfRuntime('GET', `/workers/scripts/ads-operations-sync-prod/versions/${encodeURIComponent(sync.versionId)}`),
    cfRuntime('GET', '/workers/scripts/ads-operations-sync-prod/schedules'),
  ]);
  const prodEnv = plainBindings(prodVersion?.result);
  const syncEnv = plainBindings(syncVersion?.result);
  const schedules = Array.isArray(syncSchedules?.result?.schedules)
    ? syncSchedules.result.schedules
    : Array.isArray(syncSchedules?.result)
      ? syncSchedules.result
      : null;
  ...
  const [devControl, devStore, prodControl] = await Promise.all([
    d1Query(cfPrivileged, devControlId, migrationSql),
    d1Query(cfPrivileged, devStoreId, migrationSql),
    d1Query(cfPrivileged, prodControlId, migrationSql),
  ]);
  const migrations = {
  ...
  const access = {
    ...
    nonIdentityPolicyCount: accessPolicies?._success ? policies.filter((policy) => policy?.decision === 'non_identity').length : null,
    serviceTokenCount: serviceTokens?._success ? serviceTokenList.length : null,
  };
  ...
  for (const store of stores) {
    ...
    if (exact && Number(exact.size) === store.contentBytes && String(exact.etag || '') === expectedEtag) {
  ...
  const releaseTrace = {
    development: await verifyExactArtifact({
      gh,
      owner,
      name,
      artifactName: `cloudflare-release-trace-development-${mainSha}`,
      expectedWorkflow: 'Cloudflare Release Trace',
    }),
    production: await verifyExactArtifact({
      gh,
      owner,
      name,
      artifactName: `cloudflare-release-trace-production-${mainSha}`,
      expectedWorkflow: 'Cloudflare Release Trace',
    }),
  };
...
async function collectRuntime({ cf, workerName, workerTag, buildRequired = true }) {
  ...
  const runtime = {
    workerName,
    workerTag: requiredText(workerTag, `FINAL_CLOSURE_WORKER_TAG_REQUIRED:${workerName}`),
    deploymentId: requiredText(active.id, `FINAL_CLOSURE_DEPLOYMENT_REQUIRED:${workerName}`),
    versionId,
    traffic,
  };
  ...
  return {
    ...runtime,
    buildUuid: requiredText(build.build_uuid, `FINAL_CLOSURE_BUILD_UUID_REQUIRED:${workerName}`),
    buildOutcome: String(build.build_outcome || ''),
    sourceCommit: requireSha(build?.build_trigger_metadata?.commit_hash, `FINAL_CLOSURE_SOURCE_SHA_INVALID:${workerName}`),
...
function rows(result, index) {
  return Array.isArray(result?.[index]?.results) ? result[index].results : [];
}
...
async function verifyExactArtifact({ gh, owner, name, artifactName, expectedWorkflow }) {
  const artifacts = await findArtifacts({
    ...
    owner,
    name,
    predicate: (artifact) => artifact?.name === artifactName,
    maxPages: 10,
  });
  for (const artifact of artifacts) {
    ...
    if (run?.conclusion === 'success' && run?.name === expectedWorkflow) {
      return {
        verified: true,
...
async function verifyLatestAcceptanceArtifact({ gh, owner, name }) {
  const artifacts = await findArtifacts({
    ...
    owner,
    name,
    predicate: (artifact) => String(artifact?.name || '').startsWith('live-human-review-service-auth-acceptance-'),
    maxPages: 10,
  });
  ...
  for (const artifact of artifacts) {
    ...
    if (
      ...
      && run?.head_branch === EXPECTED.acceptanceBranch
      && String(run?.head_sha || '').toLowerCase() === EXPECTED.acceptanceHeadSha
    ) {
      return {
        verified: true,
        artifact: artifact.name,
...
async function cloudflareStrict(options) {
  const payload = await cloudflareSoft(options);
  if (!payload._success) {
    throw new Error(`FINAL_CLOSURE_CLOUDFLARE_REQUEST_FAILED:${payload._httpStatus}:${options.path}`);
  }
  return payload;
}

async function cloudflareSoft({ fetchImpl, accountId, token, method, path, body }) {
...
function d1Bindings(version) {
  const out = {};
  for (const list of [version?.resources?.bindings, version?.bindings]) {
    if (!Array.isArray(list)) continue;
    for (const binding of list) {
      if (binding?.type === 'd1' && binding?.name && binding?.id) out[binding.name] = String(binding.id);
    }
  }
  return out;
}
...
function booleanBinding(value, key) {
  if (value === 'false' || value === false) return false;
  if (value === 'true' || value === true) return true;
  throw new Error(`FINAL_CLOSURE_BINDING_INVALID:${key}`);
}

function bindingText(value, key) {
  if (typeof value !== 'string') throw new Error(`FINAL_CLOSURE_BINDING_MISSING:${key}`);
  return value;
}
