...
assert.match(query, /const BID_GOVERNANCE_PARITY_AUDIT_VERSION = '1\.0\.4'/);
...
assert.match(parityAudit, /const AUDIT_VERSION = '1\.0\.4'/);
assert.match(legacyCore, /getBidGovernanceScopedRowsForParity/);
...
assert.match(parityAudit, /const metricParityPass = totalsPass && identityPass/);
assert.match(parityAudit, /const migrationBlockers = requiredReadiness\.filter/);
assert.match(parityAudit, /legacyBidComparable/);
assert.match(parityAudit, /comparison\.bidControlParityPass/);
...
await import('./test-cloudflare-access-identity.mjs');
