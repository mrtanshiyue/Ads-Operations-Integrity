(function initCloudflareCsvProductUi(global) {
  ...
  function mount() {
    ...
    state.observer = new MutationObserver(() => ensureDataNavigation());
  ...
  function ensureDataNavigation() {
    ...
    // The old Imports console appended a best-effort button under Operations. Once the
    // product group exists it is deliberately removed so CSV entrypoints have one owner.
    groupsHost.querySelectorAll('[data-csv-import-nav]').forEach((node) => node.remove());
    ...
    group.innerHTML = `<div class="cfOperatorGroupLabel">${locale === 'en' ? 'Data' : '数据'}</div><div class="cfOperatorGroupItems">${entries.map((entry) => {
  ...
  async function onDocumentClick(event) {
    ...
    if (nav) {
      ...
      const key = nav.dataset.csvProductNav;
      if (key === 'imports') return void openImports();
      if (key === 'intelligence') return void openCsvIntelligence();
      if (key === 'advisory') return void openAdvisoryReview();
    }
    ...
    if (transition) return void transitionReview(transition.dataset.reviewState);
  }

  function onDocumentChange(event) {
    if (!state.reviewRoot?.contains(event.target)) return;
    if (event.target?.id === 'cfAdvisoryStateFilter') void refreshAdvisoryReview();
  }
