(function initCloudflareNativeOperationsHealth(global) {
  ...
  const WORK_QUEUE_SCHEMA_VERSION = 'daily-operator-work-queue-v1';
  ...
  function decisionDataHealth(startDate, endDate) {
    ...
    return api().analyticsDataHealth({ includeDecisionQueue: true, startDate: range.startDate, endDate: range.endDate });
  }
  ...
  function classifyStoreHealth(store, failures = [], loadError = null) {
    ...
    if (['failed', 'error', 'blocked'].includes(syncStatus)) return attention(ATTENTION.FAILURE, 'health_failure', 'P1 Health failure', `Sync status: ${syncStatus}${sync.lastErrorCode ? ` · ${sync.lastErrorCode}` : ''}`, mapping);
    const lastSuccessMs = timestampMs(sync.lastSuccessAt);
    const lastErrorMs = timestampMs(sync.lastErrorAt);
    if (lastErrorMs && (!lastSuccessMs || lastErrorMs > lastSuccessMs)) return attention(ATTENTION.FAILURE, 'health_failure', 'P1 Health failure', `Latest sync evidence is an error${sync.lastErrorCode ? ` · ${sync.lastErrorCode}` : ''}`, mapping);
  ...
  function buildCommandRow(storeMeta, payload, loadError = null, storeOrder = 0) {
    ...
    const failures = (Array.isArray(payload?.recentRollupFailures) ? payload.recentRollupFailures : []).filter((row) => !row?.storeId || String(row.storeId) === String(storeMeta?.storeId || ''));
  ...
  function classifyDecisionAttention(store, loadError = null) {
    if (loadError || !store || store.unavailable === true || store.evidenceState !== 'available') {
      return decisionAttention(DECISION_ATTENTION.FAILURE, 'decision_evidence_gap', 'P1 Evidence gap', loadError ? `Decision read failed: ${errorText(loadError)}` : `Decision evidence unavailable${store?.error?.code ? ` · ${store.error.code}` : ''}`);
    }
    const needsReview = safeCount(store.needsReviewCount);
    const stale = safeCount(store.staleReviewEvidenceCount);
    if (needsReview > 0 || stale > 0) return decisionAttention(DECISION_ATTENTION.REVIEW, 'review_attention', 'P2 Review attention', `${needsReview} needs review · ${stale} stale evidence`);
    ...
    return decisionAttention(DECISION_ATTENTION.CLEAR, 'queue_clear', 'P5 Queue clear', safeCount(store.acknowledgedCount) > 0 ? 'Acknowledged-only; no active decision queue' : 'No active decision queue');
  }
  ...
  function buildDecisionRow(store, storeOrder = 0, loadError = null) {
    ...
    return {
      ...classification,
      storeId: String(store?.storeId || ''), storeCode: String(store?.storeCode || ''), displayName: String(store?.displayName || ''), storeOrder,
  ...
  function decisionAttention(decisionPriority, decisionKey, decisionLabel, decisionReason) { return Object.freeze({ decisionPriority, decisionKey, decisionLabel, decisionReason }); }
  ...
  const publicApi = Object.freeze({
    ...
    workQueueSchemaVersion: WORK_QUEUE_SCHEMA_VERSION,
    dataHealth, decisionDataHealth, auditEvents, listStores, capabilities,
    classifyStoreHealth, rankStoreHealthRows, buildCommandRow,
    classifyDecisionAttention, buildDecisionRow, rankDecisionRows,
    mount, open,
  });

  Object.defineProperty(global, 'CloudflareOperationsHealth', { value: publicApi, writable: false, configurable: false, enumerable: true });
  ...
  if (global.document.readyState === 'loading') global.document.addEventListener('DOMContentLoaded', mount, { once: true }); else mount();
  ...
  function mount() {
    ...
    button.id = 'btnNativeOperationsHealth'; button.type = 'button'; button.className = 'btn'; button.textContent = '运营总览';
    button.title = '查看 Four-Store Command Board 的 Operational Health、Daily Operator Work Queue 与 Decision Workload';
    button.style.display = 'none'; button.addEventListener('click', open); host.insertBefore(button, host.firstChild);

    const modal = global.document.createElement('div');
    modal.id = 'nativeOperationsHealthModal'; modal.className = 'modalOverlay cfOpsHealthOverlay';
    modal.setAttribute('role', 'dialog'); modal.setAttribute('aria-modal', 'true'); modal.setAttribute('aria-labelledby', 'nativeOperationsHealthTitle');
    modal.innerHTML = `
      <div class="largeModal cfOpsHealthModal">
        <div class="largeModalHeader cfOpsHealthHeader"><div><div class="cfOpsHealthEyebrow">PRODUCTIZATION · FOUR-STORE COMMAND BOARD · READ ONLY</div><h2 id="nativeOperationsHealthTitle">Four-Store Command Board</h2><div class="small">Operational Attention、Daily Operator Work Queue 与 Decision Attention 保持分离；不生成 Store Score，不推断隐藏时间窗口，不执行任何 Amazon mutation。</div></div><div class="cfOpsHealthHeaderActions"><span id="cfOpsHealthAccess" class="cfOpsHealthAccess">只读</span><button id="btnCfOpsHealthRefresh" class="btn" type="button">刷新健康</button><button id="btnCfOpsHealthClose" class="btn" type="button">关闭</button></div></div>
        <div class="largeModalBody cfOpsHealthBody">
          <section class="cfOpsHealthSection cfOpsHealthOverview" aria-labelledby="cfOpsHealthOverviewHeading"><div class="cfOpsHealthSectionHead"><div><div class="cfOpsHealthKicker">OPERATIONAL ATTENTION ORDER</div><h3 id="cfOpsHealthOverviewHeading">跨店运营处理顺序</h3></div><span class="small" id="cfOpsHealthOverviewGenerated">尚未读取</span></div><div class="cfOpsHealthSummary" id="cfOpsHealthSummary"></div><div class="table-container cfOpsHealthTableWrap cfOpsHealthOverviewWrap"><table class="cfOpsHealthTable"><thead><tr><th>Priority</th><th>Store</th><th>Why</th><th>Lag</th><th>Mapping</th><th>Last success</th><th>Action</th></tr></thead><tbody id="cfOpsHealthOverviewRows"></tbody></table></div><div class="small cfOpsHealthRule">Operational order: health failure / evidence gap → reported lag → unresolved mapping → healthy. “Never synced” without positive freshness evidence is fail-closed, never Healthy.</div></section>

          <section class="cfOpsHealthSection cfOpsDecisionSection" aria-labelledby="cfOpsDecisionHeading">
            <div class="cfOpsHealthSectionHead"><div><div class="cfOpsHealthKicker">DECISION SCOPE</div><h3 id="cfOpsDecisionHeading">显式 Decision 日期范围</h3></div><span class="small" id="cfOpsDecisionGenerated">显式日期范围必填</span></div>
            <div class="cfOpsDecisionControls"><label>Start Date<input id="cfOpsDecisionStart" type="date" autocomplete="off"></label><label>End Date<input id="cfOpsDecisionEnd" type="date" autocomplete="off"></label><button id="btnCfOpsDecisionLoad" class="btn" type="button">读取 Work Queue</button><span class="small">不默认当前月 / 7 天 / 30 天</span></div>

            <div class="cfOpsWorkBlock"><div class="cfOpsHealthSectionHead"><div><div class="cfOpsHealthKicker">DAILY OPERATOR WORK QUEUE</div><h3>今天先处理什么</h3></div><span class="small">server-authoritative order</span></div><div class="cfOpsHealthSummary" id="cfOpsWorkSummary"></div><div class="table-container cfOpsHealthTableWrap cfOpsHealthOverviewWrap"><table class="cfOpsHealthTable"><thead><tr><th>Operational Priority</th><th>Store</th><th>Why</th><th>Needs Review</th><th>Stale</th><th>High</th><th>Other</th><th>Action</th></tr></thead><tbody id="cfOpsWorkRows"></tbody></table></div><div class="small cfOpsHealthRule">Work Queue order comes from the server contract only: authoritative read failure/evidence gap → stale review → needs review → critical/high unreviewed → other unreviewed → acknowledged-only/no active queue. No Store Score.</div></div>

            <div class="cfOpsWorkBlock"><div class="cfOpsHealthSectionHead"><div><div class="cfOpsHealthKicker">DECISION ATTENTION</div><h3>跨店 Decision Workload</h3></div></div><div class="cfOpsHealthSummary" id="cfOpsDecisionSummary"></div><div class="table-container cfOpsHealthTableWrap cfOpsHealthOverviewWrap"><table class="cfOpsHealthTable"><thead><tr><th>Decision Priority</th><th>Store</th><th>Why</th><th>Decision Queue</th><th>Needs Review</th><th>High Unreviewed</th><th>Stale Evidence</th><th>Action</th></tr></thead><tbody id="cfOpsDecisionRows"></tbody></table></div><div class="small cfOpsHealthRule">Decision order: authoritative read failure/evidence gap → needs-review or stale-review evidence → unreviewed critical/high → other unreviewed → acknowledged-only / no active queue. No opaque combined score.</div></div>
          </section>

          <div class="cfOpsHealthControls"><label>当前检查店铺<select id="cfOpsHealthStore"></select></label><div class="small cfOpsHealthGenerated" id="cfOpsHealthGenerated">尚未读取</div></div><div id="cfOpsHealthStatus" class="cfOpsHealthStatus" aria-live="polite"></div>
          ...
          <div class="small cfOpsHealthFoot">数据来源仅为 same-origin stores、capabilities、analyticsDataHealth 与 auditEvents。Daily Operator Work Queue 与 Decision Workload 复用 server-authoritative recommendation / durable review truth；本面板不启动 sync、不写 D1、不执行 Amazon mutation、不执行 deployment。</div>
        </div>
      </div>`;
  ...
  async function probeAccess() {
    try {
      ...
      state.stores = normalizeStores(storesPayload?.stores).filter((store) => canReadAnalytics(store.storeId));
      if (!state.stores.some((store) => store.storeId === state.storeId)) state.storeId = state.stores[0]?.storeId || '';
      renderStores(); renderAccess();
      const button = global.document.querySelector('#btnNativeOperationsHealth'); if (button) button.style.display = state.stores.length ? '' : 'none';
    } catch {
      state.capabilities = null; state.stores = []; state.storeId = '';
      const button = global.document.querySelector('#btnNativeOperationsHealth'); if (button) button.style.display = 'none';
    }
  }
  ...
  async function open() {
    ...
    const modal = global.document.querySelector('#nativeOperationsHealthModal'); if (!modal) return;
  ...
  async function refreshDecisionQueue(options = {}) {
    ...
    if (!range) {
      state.decisionRows = []; state.operatorWorkRows = []; state.decisionGeneratedAt = ''; state.decisionRange = null; state.decisionError = 'decision_queue_date_range_required';
      renderDecisionQueue();
      if (!options.quiet) setStatus('Decision Queue 必须显式提供 Start Date 和 End Date；未使用任何默认时间窗口', 'bad');
      return;
    }
    const serial = ++state.decisionSerial;
    state.decisionLoading = true; setDecisionBusy(true);
    if (!options.quiet) setStatus(`正在读取 ${range.startDate} → ${range.endDate} 的 server-authoritative Daily Operator Work Queue…`);
    try {
      ...
      if (payload?.decisionQueue?.authority?.readOnly !== true || payload?.decisionQueue?.authority?.executionAuthorized !== false || payload?.decisionQueue?.authority?.amazonMutationAuthorized !== false) throw Object.assign(new Error('decision_queue_authority_invalid'), { code: 'decision_queue_authority_invalid' });
      if (payload?.operatorWorkQueue?.schemaVersion !== WORK_QUEUE_SCHEMA_VERSION) throw Object.assign(new Error('operator_work_queue_contract_unavailable'), { code: 'operator_work_queue_contract_unavailable' });
      if (payload?.operatorWorkQueue?.authority?.readOnly !== true || payload?.operatorWorkQueue?.authority?.executionAuthorized !== false || payload?.operatorWorkQueue?.authority?.amazonMutationAuthorized !== false) throw Object.assign(new Error('operator_work_queue_authority_invalid'), { code: 'operator_work_queue_authority_invalid' });
      state.operatorWorkRows = Array.isArray(payload.operatorWorkQueue.rows) ? payload.operatorWorkQueue.rows.slice() : [];
      ...
      state.decisionGeneratedAt = payload.operatorWorkQueue.generatedAt || payload.decisionQueue.generatedAt || payload.generatedAt || '';
      state.decisionRange = range; state.decisionError = null; renderDecisionQueue();
      const gaps = state.operatorWorkRows.filter((row) => row.priority === 1).length;
      if (!options.quiet) setStatus(gaps ? `${gaps} 个店铺 Work Queue evidence fail-closed；已排到 Daily Operator Work Queue 最前` : 'Daily Operator Work Queue 已刷新', gaps ? 'bad' : 'ok');
    } catch (error) {
      if (serial !== state.decisionSerial) return;
      state.decisionRows = []; state.operatorWorkRows = []; state.decisionGeneratedAt = ''; state.decisionRange = range; state.decisionError = errorText(error); renderDecisionQueue();
      if (!options.quiet) setStatus(`Daily Operator Work Queue 读取失败 · ${errorText(error)}`, 'bad');
    } finally { if (serial === state.decisionSerial) { state.decisionLoading = false; setDecisionBusy(false); } }
  }

  function storeOrder(storeId) { const index = state.stores.findIndex((store) => store.storeId === storeId); return index >= 0 ? index : Number.MAX_SAFE_INTEGER; }
  async function selectStore(storeId) {
    const id = String(storeId || '');
    if (!id || !state.stores.some((store) => store.storeId === id) || !canReadAnalytics(id)) return;
    state.storeId = id; state.health = state.healthByStore[id] || null; renderStores(); renderAccess(); renderHealth();
  ...
  function renderOverview() {
    ...
    if (summary) {
      ...
      for (const [label, value, tone] of [['P1 Health / evidence', state.overview.filter((row) => row.priority === ATTENTION.FAILURE).length, 'p1'], ['P2 Freshness', state.overview.filter((row) => row.priority === ATTENTION.FRESHNESS).length, 'p2'], ['P3 Mapping', state.overview.filter((row) => row.priority === ATTENTION.MAPPING).length, 'p3'], ['P4 Healthy', state.overview.filter((row) => row.priority === ATTENTION.HEALTHY).length, 'p4']]) summary.appendChild(summaryChip(label, value, tone));
    }
    const tbody = global.document.querySelector('#cfOpsHealthOverviewRows'); if (!tbody) return;
    tbody.replaceChildren(); if (!state.overview.length) return appendEmpty(tbody, 7, '当前身份没有可读店铺健康证据');
    for (const row of state.overview) {
      const tr = global.document.createElement('tr'); const priority = textCell(row.label); priority.dataset.priority = String(row.priority); tr.appendChild(priority);
      tr.appendChild(textCell([row.displayName || row.storeCode || row.storeId, row.marketplaceCode].filter(Boolean).join('\n'))); tr.appendChild(textCell(row.reason)); tr.appendChild(textCell(row.lagMinutes === null ? '—' : `${formatNumber(row.lagMinutes)} min`)); tr.appendChild(textCell(`${row.unmappedRows} / ${row.ambiguousRows}`)); tr.appendChild(textCell(row.lastSuccessAt || '—'));
      const action = global.document.createElement('td'); const inspect = buttonNode(row.storeId === state.storeId ? '当前店铺' : '查看店铺', () => { void selectStore(row.storeId); }); inspect.disabled = row.storeId === state.storeId; action.appendChild(inspect); tr.appendChild(action); tbody.appendChild(tr);
    }
  }
  ...
  function renderDecisionQueue() {
    ...
    const tbody = global.document.querySelector('#cfOpsDecisionRows'); if (!tbody) return;
  ...
  function renderOperatorWorkQueue() {
    const summary = global.document.querySelector('#cfOpsWorkSummary');
    if (summary) {
      summary.replaceChildren();
      if (state.operatorWorkRows.length) {
        summary.appendChild(summaryChip('P1 evidence', state.operatorWorkRows.filter((row) => row.priority === 1).length, 'p1'));
        summary.appendChild(summaryChip('P2 review', state.operatorWorkRows.filter((row) => row.priority === 2).length, 'p2'));
        summary.appendChild(summaryChip('P3 high', state.operatorWorkRows.filter((row) => row.priority === 3).length, 'p3'));
        summary.appendChild(summaryChip('P4/P5', state.operatorWorkRows.filter((row) => row.priority >= 4).length, 'p4'));
      }
    }
    const tbody = global.document.querySelector('#cfOpsWorkRows'); if (!tbody) return;
    tbody.replaceChildren();
    if (!state.decisionRange) return appendEmpty(tbody, 8, '输入显式 Start Date / End Date 后读取 Daily Operator Work Queue');
    if (state.decisionError) return appendEmpty(tbody, 8, `Operator Work Queue unavailable · ${state.decisionError}`);
    if (!state.operatorWorkRows.length) return appendEmpty(tbody, 8, '当前身份在该显式时间范围没有 server-authoritative Work Queue rows');
  ...
  function renderHealth() {
    const payload = state.health || {}; const store = Array.isArray(payload.stores) ? payload.stores[0] : null; const sync = store?.sync || {};
    const generated = global.document.querySelector('#cfOpsHealthGenerated'); if (generated) generated.textContent = payload.generatedAt ? `Generated ${payload.generatedAt}` : '尚无健康数据';
    const pill = global.document.querySelector('#cfOpsHealthSyncPill'); if (pill) { pill.textContent = String(sync.status || 'unknown'); pill.dataset.status = String(sync.status || 'unknown').toLowerCase(); }
    const cards = global.document.querySelector('#cfOpsHealthCards');
    if (cards) { cards.replaceChildren(); for (const [label, value] of [['Store', store?.displayName || store?.storeCode || state.storeId || '—'], ['Lag', sync.lagMinutes === null || sync.lagMinutes === undefined ? '—' : `${sync.lagMinutes} min`], ['Last success', sync.lastSuccessAt || '—'], ['Last error', sync.lastErrorAt || '—'], ['Error code', sync.lastErrorCode || '—'], ['Active run', sync.activeRunId || '—']]) cards.appendChild(metricCard(label, value)); }
    renderRollups(Array.isArray(store?.rollups) ? store.rollups : []); renderFailures((Array.isArray(payload.recentRollupFailures) ? payload.recentRollupFailures : []).filter((row) => !row.storeId || row.storeId === state.storeId));
  }
  function renderRollups(rows) {
    const tbody = global.document.querySelector('#cfOpsHealthRollups'); if (!tbody) return; tbody.replaceChildren(); if (!rows.length) return appendEmpty(tbody, 8, '当前店铺没有 rollup watermark');
    for (const row of rows) { const tr = global.document.createElement('tr'); for (const value of [row.rollupType || '—', row.partitionKey || '—', row.lastSuccessDate || '—', row.lastSuccessAsOfDate || '—', row.summaryRows ?? '—']) tr.appendChild(textCell(value)); tr.appendChild(numberCell(row.unmappedRows)); tr.appendChild(numberCell(row.ambiguousRows)); tr.appendChild(textCell([row.lastSuccessRunId, row.updatedAt].filter(Boolean).join('\n') || '—')); tbody.appendChild(tr); }
  }
  function renderFailures(rows) {
    const tbody = global.document.querySelector('#cfOpsHealthFailures'); if (!tbody) return; tbody.replaceChildren(); if (!rows.length) return appendEmpty(tbody, 5, '当前没有最近 Rollup 失败记录');
    for (const row of rows) { const tr = global.document.createElement('tr'); for (const value of [row.startedAt || '—', row.rollupType || '—', row.partitionKey || '—', row.errorCode || '—', row.completedAt || '—']) tr.appendChild(textCell(value)); tbody.appendChild(tr); }
  }
  function renderAudits(allowed) {
    ...
    const tbody = global.document.querySelector('#cfOpsHealthAudits'); if (!tbody) return; tbody.replaceChildren(); if (!allowed) return appendEmpty(tbody, 5, '无 audit.read；健康数据仍保持可读'); if (!state.audits.length) return appendEmpty(tbody, 5, '当前店铺没有最近审计事件');
    for (const row of state.audits) { const tr = global.document.createElement('tr'); for (const value of [row.occurredAt || '—', row.actor?.displayName || row.actor?.email || row.actor?.userId || 'system', row.action || '—', [row.entityType, row.entityId].filter(Boolean).join(' · ') || '—', row.requestId || row.cfRay || '—']) tr.appendChild(textCell(value)); tbody.appendChild(tr); }
  }
  function renderStores() {
    const select = global.document.querySelector('#cfOpsHealthStore'); if (!select) return; const current = state.storeId; select.replaceChildren();
    for (const store of state.stores) { const option = global.document.createElement('option'); option.value = store.storeId; option.textContent = [store.displayName || store.storeCode || store.storeId, store.marketplaceCode].filter(Boolean).join(' · '); option.selected = store.storeId === current; select.appendChild(option); } select.disabled = !state.stores.length;
  }
  function renderAccess() {
    const badge = global.document.querySelector('#cfOpsHealthAccess'); if (!badge) return; const analytics = canReadAnalytics(state.storeId); const audit = canReadAudit(state.storeId); const readableStores = state.stores.filter((store) => canReadAnalytics(store.storeId)).length; badge.textContent = analytics ? `${readableStores} stores · ${audit ? 'analytics.read + audit.read' : 'analytics.read'}` : '无权限'; badge.dataset.mode = analytics ? (audit ? 'full' : 'health') : 'none';
  }

  function canReadAnalytics(storeId) { if (!storeId && !state.capabilities) return false; if (globalPermissions().has('analytics.read')) return true; return storePermissions(storeId).has('analytics.read'); }
  ...
  function normalizeStores(rows) { return (Array.isArray(rows) ? rows : []).map((row) => ({ storeId: String(row.storeId || row.store_id || ''), storeCode: String(row.storeCode || row.store_code || ''), displayName: String(row.displayName || row.display_name || ''), marketplaceCode: String(row.marketplaceCode || row.marketplace_code || '') })).filter((row) => row.storeId); }
  function normalizeDecisionRange(startDate, endDate) { const start = String(startDate || '').trim(); const end = String(endDate || '').trim(); if (!/^\d{4}-\d{2}-\d{2}$/u.test(start) || !/^\d{4}-\d{2}-\d{2}$/u.test(end) || Number.isNaN(Date.parse(`${start}T00:00:00.000Z`)) || Number.isNaN(Date.parse(`${end}T00:00:00.000Z`)) || start > end) return null; return { startDate: start, endDate: end }; }
  ...
  function metricCard(label, value) { const card = global.document.createElement('div'); card.className = 'cfOpsHealthCard'; const key = global.document.createElement('div'); key.className = 'cfOpsHealthCardKey'; key.textContent = label; const val = global.document.createElement('div'); val.className = 'cfOpsHealthCardValue'; val.textContent = String(value ?? '—'); card.append(key, val); return card; }
  ...
  function setBusy(value, message) { state.loading = Boolean(value); const modal = global.document.querySelector('#nativeOperationsHealthModal'); if (modal) modal.setAttribute('aria-busy', state.loading || state.decisionLoading ? 'true' : 'false'); const refreshButton = global.document.querySelector('#btnCfOpsHealthRefresh'); if (refreshButton) refreshButton.disabled = state.loading; const storeSelect = global.document.querySelector('#cfOpsHealthStore'); if (storeSelect) storeSelect.disabled = state.loading || !state.stores.length; if (message) setStatus(message); }
  function setDecisionBusy(value) { const button = global.document.querySelector('#btnCfOpsDecisionLoad'); if (button) button.disabled = Boolean(value); const start = global.document.querySelector('#cfOpsDecisionStart'); if (start) start.disabled = Boolean(value); const end = global.document.querySelector('#cfOpsDecisionEnd'); if (end) end.disabled = Boolean(value); const modal = global.document.querySelector('#nativeOperationsHealthModal'); if (modal) modal.setAttribute('aria-busy', state.loading || Boolean(value) ? 'true' : 'false'); }
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .cfOpsHealthCards{display:grid;grid-template-columns:repeat(6,minmax(120px,1fr));gap:8px}.cfOpsHealthCard{border:1px solid var(--line);border-radius:10px;padding:9px;background:var(--input-bg);min-width:0}.cfOpsHealthCardKey{font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.06em}.cfOpsHealthCardValue{margin-top:5px;font-size:11px;font-weight:700;overflow-wrap:anywhere}.cfOpsHealthTableWrap{max-height:280px;overflow:auto}.cfOpsHealthOverviewWrap{max-height:320px}.cfOpsHealthFailureWrap,.cfOpsHealthAuditWrap{max-height:190px}.cfOpsHealthTable{width:100%;border-collapse:collapse}.cfOpsHealthTable th,.cfOpsHealthTable td{padding:8px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top;font-size:10px}.cfOpsHealthTable th{position:sticky;top:0;background:var(--card);z-index:1;color:var(--muted)}.cfOpsHealthTable td[data-alert="true"]{color:var(--bad);font-weight:800}.cfOpsHealthEmpty{text-align:center!important;color:var(--muted);padding:18px!important}.cfOpsHealthFoot{color:var(--muted)}
