(function initCloudflareNativeImportsConsole(global) {
  ...
  async function onSubmit(event) {
    ...
    if (file.size <= 0 || file.size > MAX_BYTES) return setMessage('bad', t('CSV 必须大于 0 且不超过 10 MB。', 'CSV must be larger than 0 and no more than 10 MB.'));
    ...
    setMessage('loading', t('正在校验并写入真实广告数据…', 'Validating and publishing real ad data…'));
    try {
      ...
      if (payload?.duplicate) {
        ...
        setMessage('ok', t(`导入成功：${payload.validation?.acceptedRows || 0} 行已发布。`, `Import complete: ${payload.validation?.acceptedRows || 0} rows published.`));
      } else {
  ...
  function uploadMarkup() {
    ...
    return `<section class="cfImportsCard cfImportsUpload">
      <div class="cfImportsSectionHead"><div><h3>${e(t('导入 Search Term CSV', 'Import Search Term CSV'))}</h3><p>${e(t('Amazon Ads Console 导出 · 最大 10 MB · 重复报告自动识别', 'Amazon Ads Console export · max 10 MB · duplicate reports detected automatically'))}</p></div><span class="cfImportsPermission">${state.canWrite ? 'ads.write' : 'read only'}</span></div>
      <form id="cfImportForm">
        <label class="cfImportsFile"><input id="cfImportFile" type="file" accept=".csv,text/csv" ${disabled ? 'disabled' : ''}><span>${e(t('选择 CSV 文件', 'Choose CSV file'))}</span><small id="cfImportFileMeta">${e(t('尚未选择 CSV', 'No CSV selected'))}</small></label>
  ...
  function detailMarkup() {
    if (!state.selectedImportId) return empty(t('选择一条导入记录查看校验与发布结果。', 'Select an import to inspect validation and publish results.'));
