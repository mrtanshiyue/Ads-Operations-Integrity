(function initCloudflareCsvAnalyticsDrilldown(global) {
  ...
  Object.defineProperty(global, 'CloudflareCsvAnalyticsDrilldown', {
    value: publicApi,
    writable: false,
    configurable: false,
    enumerable: true,
  ...
  function mount() {
    ...
    root.innerHTML = `
      <div class="cfCsvDrilldownHead">
        <div><strong>Hierarchy drill-down</strong><span>Campaign → Ad Group → Targeting → Search Term</span></div>
        <span class="cfCsvDrilldownAuthority">observed CSV identity · read only</span>
      </div>
      ...
      <div class="cfCsvDrilldownPager">
        <button type="button" class="btn" data-cfdd-prev>Previous</button>
        <span data-cfdd-page>Page —</span>
        <button type="button" class="btn" data-cfdd-next>Next</button>
      </div>
    ...
    if (qualityCard) qualityCard.insertAdjacentElement('beforebegin', root);
    else dashboard.appendChild(root);
    state.root = root;
    state.mounted = true;

    root.addEventListener('change', (event) => {
      if (event.target.matches('[data-cfdd-match]')) {
        state.matchType = MATCH_TYPES.includes(event.target.value) ? event.target.value : '';
        state.page = 1;
        void refresh();
      } else if (event.target.matches('[data-cfdd-sort]')) {
        state.sort = SORTS.includes(event.target.value) ? event.target.value : 'spendMicros';
        state.page = 1;
        void refresh();
      } else if (event.target.matches('[data-cfdd-direction]')) {
    ...
    root.addEventListener('click', (event) => {
      ...
      if (event.target.closest('[data-cfdd-prev]')) {
        if (state.page <= 1) return;
        state.page -= 1;
        return void refresh();
      }
      if (event.target.closest('[data-cfdd-next]')) {
        state.page += 1;
        return void refresh();
      }
  ...
  async function refresh() {
    ...
    if (!scope.storeId || !scope.startDate || !scope.endDate) {
      renderStatus('Store and date range are required before drill-down.', 'warn');
      return;
    }
    ...
    try {
      ...
      const scopeParams = buildRequestParams({
        startDate: scope.startDate,
        endDate: scope.endDate,
        matchType: state.matchType,
        ...hierarchyFilters(),
  ...
  function reset(refreshNow = true) {
    ...
    state.q = '';
    if (state.root) {
      const match = state.root.querySelector('[data-cfdd-match]');
      const sort = state.root.querySelector('[data-cfdd-sort]');
      const direction = state.root.querySelector('[data-cfdd-direction]');
  ...
  function buildRequestParams(input = {}) {
    const out = {};
    for (const key of ['startDate', 'endDate', 'campaignId', 'adGroupId', 'targetingId', 'matchType', 'marketplace', 'profileId', 'advertiserAccountId', 'q', 'sort', 'direction']) {
  ...
  function nextSelectionFromRow(row) {
    const level = String(row.dataset.cfddLevel || '');
    const id = String(row.dataset.cfddId || '').trim();
    const label = String(row.dataset.cfddLabel || '').trim();
    if (!id || !LEVELS.includes(level) || level === 'search-term') return null;
    return Object.freeze({ level, id, label: label || id });
  }

  function renderPath() {
    const node = state.root?.querySelector('[data-cfdd-path]');
    if (!node) return;
    ...
    state.path.forEach((item, index) => {
      ...
      chips.push(`<button type="button" data-cfdd-depth="${index + 1}" class="cfCsvDrilldownChip${index === state.path.length - 1 ? ' active' : ''}">${escapeHtml(levelLabel(item.level))}: ${escapeHtml(item.label)}</button>`);
    });
    if (state.matchType) {
      chips.push('<span>·</span>');
      chips.push(`<span class="cfCsvDrilldownFilterChip">Match type: ${escapeHtml(state.matchType)}</span>`);
    }
    node.innerHTML = chips.join('');
  }

  function renderScope(overview, quality) {
    const node = state.root?.querySelector('[data-cfdd-scope]');
    if (!node) return;
    if (!overview?.metrics) {
      node.innerHTML = '';
      return;
    ...
    node.innerHTML = `
      ${scopeCard('Spend', moneyMicros(metrics.spendMicros), 'Scoped total, page-independent')}
      ${scopeCard('Sales', moneyMicros(metrics.salesMicros), 'Scoped total, page-independent')}
      ${scopeCard('Orders', formatInt(metrics.orders), 'Scoped total, page-independent')}
      ${scopeCard('ACoS', percent(metrics.acos), 'Scoped ratio after aggregation')}
      ${scopeCard('ROAS', ratio(metrics.roas), 'Scoped ratio after aggregation')}
      ${scopeCard('Quality', q.qualityScore == null ? '—' : Number(q.qualityScore).toFixed(1), `${formatInt(q.issueCount)} issue types · reliability only`)}`;
  }

  function renderTable(payload) {
    const head = state.root?.querySelector('[data-cfdd-head]');
    const body = state.root?.querySelector('[data-cfdd-body]');
    if (!head || !body) return;
    const items = Array.isArray(payload?.items) ? payload.items : [];
    const columns = columnsFor(state.level);
    head.innerHTML = `<tr>${columns.map((column) => `<th>${escapeHtml(column.label)}</th>`).join('')}</tr>`;
    body.innerHTML = items.length
      ? items.map((item) => rowMarkup(item, columns)).join('')
      : `<tr><td colspan="${columns.length}" class="cfCsvDrilldownEmpty">No matching governed business facts.</td></tr>`;

    const paging = payload?.pagination || {};
    const page = Number(paging.page || 1);
    const totalPages = Number(paging.totalPages || 0);
    state.page = page;
    const pageNode = state.root.querySelector('[data-cfdd-page]');
    if (pageNode) pageNode.textContent = totalPages ? `Page ${page} / ${totalPages} · ${formatInt(paging.totalItems)} rows` : 'Page —';
    const prev = state.root.querySelector('[data-cfdd-prev]');
    const next = state.root.querySelector('[data-cfdd-next]');
    if (prev) prev.disabled = totalPages === 0 || page <= 1;
    if (next) next.disabled = totalPages === 0 || page >= totalPages;
  }

  function rowMarkup(item, columns) {
    const identity = drillIdentity(item, state.level);
    const drillable = Boolean(identity.id && state.level !== 'search-term');
    return `<tr data-cfdd-drill data-cfdd-level="${escapeHtml(state.level)}" data-cfdd-id="${escapeHtml(identity.id || '')}" data-cfdd-label="${escapeHtml(identity.label || '')}" aria-disabled="${drillable ? 'false' : 'true'}" class="${drillable ? 'drillable' : ''}">${columns.map((column) => `<td>${cell(item, column)}</td>`).join('')}</tr>`;
  }

  function drillIdentity(item, level) {
    if (level === 'campaign') return { id: item?.campaignId || null, label: item?.campaignName || null };
    if (level === 'ad-group') return { id: item?.adGroupId || null, label: item?.adGroupName || null };
    if (level === 'targeting') return { id: item?.targetingId || null, label: item?.targeting || null };
    return { id: null, label: item?.searchTerm || null };
  }
  ...
  function cell(item, column) {
    ...
    if (column.kind === 'money') return escapeHtml(moneyMicros(value));
    if (column.kind === 'int') return escapeHtml(formatInt(value));
    if (column.kind === 'percent') return escapeHtml(percent(value));
    if (column.kind === 'ratio') return escapeHtml(ratio(value));
    if (column.kind === 'identity') {
      return value
        ? `<span class="cfCsvDrilldownObservedId">${escapeHtml(value)}</span><small>observed · non-canonical</small>`
        : '<span class="cfCsvDrilldownBlocked">Unavailable · drill blocked</span>';
    }
    return escapeHtml(value == null || value === '' ? '—' : value);
  }

  function renderStatus(text, tone) {
    const node = state.root?.querySelector('[data-cfdd-status]');
    if (!node) return;
  ...
  function matchTypeOptions() {
    return MATCH_TYPES.map((value) => `<option value="${value}">${value || 'All match types'}</option>`).join('');
  }
  ...
  function sortOptions() {
    ...
    return SORTS.map((value) => `<option value="${value}">${labels[value]}</option>`).join('');
  }

  function levelLabel(level) {
    return ({ campaign: 'Campaign', 'ad-group': 'Ad Group', targeting: 'Targeting', 'search-term': 'Search Term' })[level] || 'Campaign';
  }
