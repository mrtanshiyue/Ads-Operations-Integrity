(() => {
  ...
  function rawEligibility(request = currentRequest(), cloudState = window.PrivateCloudAds?.state?.() || {}) {
    const reasons = [];
    const loadedMonths = Array.isArray(cloudState.loadedMonths) ? cloudState.loadedMonths : [];
    const range = cloudState.loadedRange && typeof cloudState.loadedRange === 'object'
      ? cloudState.loadedRange
      : null;
    const bootstrapMonths = Array.isArray(cloudState.bootstrap?.coverage?.months)
      ? [...cloudState.bootstrap.coverage.months].sort()
      : [];
    ...
    if (cloudState.rawBootstrapFingerprint && cloudState.dataFingerprint
      && cloudState.rawBootstrapFingerprint !== cloudState.dataFingerprint) {
    ...
    const requestedFromMonth = request.from ? request.from.slice(0, 7) : '';
    const requestedToMonth = request.to ? request.to.slice(0, 7) : '';
    if (range) {
      if (requestedFromMonth && requestedFromMonth < text(range.fromMonth)) {
        reasons.push(`Raw 从 ${text(range.fromMonth)} 开始，早于范围的数据未加载`);
      }
      if (requestedToMonth && requestedToMonth > text(range.toMonth)) {
        reasons.push(`Raw 到 ${text(range.toMonth)} 结束，晚于范围的数据未加载`);
      }
      if (!request.from && !request.to && bootstrapMonths.length) {
        const fullFrom = bootstrapMonths[0];
        const fullTo = bootstrapMonths.at(-1);
        if (text(range.fromMonth) !== fullFrom || text(range.toMonth) !== fullTo) {
          reasons.push(`未限定日期时必须显式加载完整历史 Raw（当前 ${text(range.fromMonth)} → ${text(range.toMonth)}）`);
        }
      }
    }
    ...
    return {
      ...
      reasons,
      loadedOnce: Boolean(cloudState.loadedOnce),
      rawStale: Boolean(cloudState.rawStale),
      loadedScope: text(cloudState.loadedScope).toUpperCase(),
      loadedMonths: [...loadedMonths],
      loadedRange: range ? {
        fromMonth: text(range.fromMonth),
        toMonth: text(range.toMonth),
        months: Array.isArray(range.months) ? [...range.months] : [],
      } : null,
      fingerprintMatch: Boolean(
        cloudState.dataFingerprint
        && cloudState.rawBootstrapFingerprint
        && cloudState.dataFingerprint === cloudState.rawBootstrapFingerprint
      ),
    };
  }
  ...
  function groupKey(row, index = 0) {
    ...
    const campaign = lower(row?.campaignId || row?.campaign);
    const adGroup = lower(row?.adGroupId || row?.adGroup);
    const targeting = lower(row?.targeting || row?.searchTerm);
    const matchType = lower(row?.matchType);
    const composite = [campaign, adGroup, targeting, matchType].join('|');
  ...
  function summarizeRows(rows, { trustedBidOnly = false } = {}) {
    ...
    input.forEach((row, index) => {
      ...
      if (!groups.has(key)) {
        groups.set(key, {
          key,
          targetingId: text(row?.targetingId),
          campaign: text(row?.campaign),
          adGroup: text(row?.adGroup),
          targeting: text(row?.targeting || row?.searchTerm),
          matchType: text(row?.matchType),
          rowCount: 0,
          impressions: 0,
          clicks: 0,
          spend: 0,
  ...
  function metricDelta(legacyValue, queryValue, rule = { abs: 0, rel: 0 }) {
    ...
    const query = number(queryValue);
    const absolute = query - legacy;
    const base = Math.max(Math.abs(legacy), Math.abs(query), 1e-9);
    const relative = Math.abs(absolute) / base;
    const pass = Math.abs(absolute) <= rule.abs || relative <= rule.rel;
    return { legacy, query, absolute, relative, pass };
  }
  ...
  function compareRows(legacyInput, queryInput) {
    ...
    const metrics = Object.fromEntries(
      Object.entries(METRIC_RULES).map(([key, rule]) => [key, metricDelta(legacy.totals[key], query.totals[key], rule)]),
    );
    const rowCount = metricDelta(legacy.totals.rowCount, query.totals.rowCount, { abs: 0, rel: 0 });
    const legacyKeys = new Set(legacy.groups.keys());
    const queryKeys = new Set(query.groups.keys());
    ...
    let bidCompared = 0;
    let bidMismatch = 0;
    let bidMissingEither = 0;

    intersection.forEach(key => {
      const left = legacy.groups.get(key);
      const right = query.groups.get(key);
      ...
      if (left.latestBid === null || right.latestBid === null) {
        if (left.latestBid !== right.latestBid) {
          bidState = 'missing';
          bidMissingEither += 1;
        }
      } else {
        bidCompared += 1;
        bidDelta = right.latestBid - left.latestBid;
        if (Math.abs(bidDelta) > 0.000001) {
          bidState = 'mismatch';
          bidMismatch += 1;
        } else {
          bidState = 'match';
      ...
      const metricMismatch = [spend, sales, orders, clicks, impressions].some(item => !item.pass);
      if (metricMismatch || bidState === 'mismatch' || bidState === 'missing') {
        mismatches.push({
          key,
          targetingId: left.targetingId || right.targetingId,
          campaign: left.campaign || right.campaign,
          adGroup: left.adGroup || right.adGroup,
          targeting: left.targeting || right.targeting,
          matchType: left.matchType || right.matchType,
          legacySpend: left.spend,
          querySpend: right.spend,
          spendDelta: spend.absolute,
          legacySales: left.sales,
          querySales: right.sales,
          salesDelta: sales.absolute,
          legacyOrders: left.orders,
          queryOrders: right.orders,
          ordersDelta: orders.absolute,
          legacyBid: left.latestBid,
          queryBid: right.latestBid,
          bidDelta,
          bidState,
        });
      }
    ...
    legacyOnly.forEach(key => {
      ...
      mismatches.push({
        key, side: 'legacy-only', targetingId: row.targetingId, campaign: row.campaign,
        adGroup: row.adGroup, targeting: row.targeting, matchType: row.matchType,
        legacySpend: row.spend, querySpend: 0, spendDelta: -row.spend,
        legacySales: row.sales, querySales: 0, salesDelta: -row.sales,
        legacyOrders: row.orders, queryOrders: 0, ordersDelta: -row.orders,
        legacyBid: row.latestBid, queryBid: null, bidDelta: null, bidState: 'missing',
      });
    });
    queryOnly.forEach(key => {
      const row = query.groups.get(key);
      mismatches.push({
        key, side: 'query-only', targetingId: row.targetingId, campaign: row.campaign,
        adGroup: row.adGroup, targeting: row.targeting, matchType: row.matchType,
        legacySpend: 0, querySpend: row.spend, spendDelta: row.spend,
        legacySales: 0, querySales: row.sales, salesDelta: row.sales,
        legacyOrders: 0, queryOrders: row.orders, ordersDelta: row.orders,
        legacyBid: null, queryBid: row.latestBid, bidDelta: null, bidState: 'missing',
      });
    });
    mismatches.sort((a, b) => Math.abs(b.spendDelta || 0) - Math.abs(a.spendDelta || 0)
      || Math.abs(b.salesDelta || 0) - Math.abs(a.salesDelta || 0));
    ...
    const near = !pass
      && Object.values(metrics).every(item => item.relative <= 0.01 || Math.abs(item.absolute) <= 1)
      && groupOverlap >= 0.98;

    return {
      verdict: pass ? 'pass' : near ? 'warn' : 'fail',
      migrationCandidate: pass,
      ...
      metrics,
      groupOverlap,
      matchedGroups: intersection.length,
      legacyOnlyCount: legacyOnly.length,
      queryOnlyCount: queryOnly.length,
      bidCompared,
      bidMismatch,
      bidMissingEither,
      legacy: legacy.totals,
      query: query.totals,
  ...
  async function run({ force = true } = {}) {
    ...
    const eligibility = rawEligibility(request);
    ...
    if (!eligibility.ready) {
      ...
      render();
      throw auditError(409, `Parity Audit 被阻止：${eligibility.reasons.join('；')}`);
    }
    if (typeof window.QueryNativeModuleData?.ads !== 'function') {
      throw auditError(503, 'Query-native Adapter 尚未就绪');
    }
    ...
    try {
      ...
      const queryRequest = { ...request, adProduct: 'SP', source: 'query', maxRows: MAX_ROWS, force };
      const payload = await window.QueryNativeModuleData.ads(queryRequest);
      if (payload?.source !== 'query-tidb') throw auditError(502, `Query 来源异常：${text(payload?.source) || 'missing'}`);
      if (payload?.truncated === true || payload?.nextOffset) {
        throw auditError(409, 'Query 结果达到分页上限，禁止用截断数据做 Parity 结论');
      }
      if (payload?.governance?.schemaVersion !== 'ads-query-governance-v2') {
        throw auditError(502, 'Query governance v2 缺失');
      }
      const queryRows = Array.isArray(payload.rows) ? payload.rows : [];
      const comparison = compareRows(legacy, queryRows);
      setState({
        status: 'ready',
        legacy: summarizeRows(legacy, { trustedBidOnly: false }).totals,
        query: summarizeRows(queryRows, { trustedBidOnly: true }).totals,
        comparison,
      ...
      render();
      dispatch('lr:bid-governance-parity-ready', snapshot());
      return snapshot();
    } catch (error) {
      setState({
        status: 'error',
      ...
      render();
      dispatch('lr:bid-governance-parity-error', {
  ...
  function snapshot() {
    return {
      ...
      eligibility: state.eligibility ? { ...state.eligibility, reasons: [...(state.eligibility.reasons || [])] } : null,
      legacy: state.legacy ? { ...state.legacy } : null,
      query: state.query ? { ...state.query } : null,
      comparison: state.comparison ? {
        ...state.comparison,
        metrics: Object.fromEntries(Object.entries(state.comparison.metrics || {}).map(([key, value]) => [key, { ...value }])),
        mismatches: (state.comparison.mismatches || []).map(row => ({ ...row })),
      } : null,
  ...
  function ensureStyles() {
    ...
    style.id = 'bidGovernanceParityAuditStyles';
    style.textContent = `
  ...
  function ensureUi() {
    ...
    ensureStyles();
    let root = byId('bidGovernanceParityAudit');
    if (!root) {
      root = document.createElement('section');
      root.id = 'bidGovernanceParityAudit';
      root.innerHTML = `
        <div class="bgpaHead">
          <div><div class="bgpaTitle">Phase 8 · Bid Governance 双源对账</div><div class="bgpaSub">Legacy Raw getBidGovScopedRows(searchTerm) ↔ TiDB Query。只审计，不自动加载 Raw，不生成调价动作，不改变执行门禁。</div></div>
          <button class="btn bgpaBtn" id="btnBidGovernanceParityAudit" type="button">运行双源对账</button>
        </div>
        <div class="bgpaBanner" id="bidGovernanceParityBanner">需要先由用户显式加载覆盖当前范围的 Raw 明细。</div>
        <div class="bgpaKpis" id="bidGovernanceParityKpis"></div>
        <div class="bgpaGrid" id="bidGovernanceParityGrid"></div>
        <div class="bgpaFoot">Parity Pass 只代表“可进入下一阶段迁移审查”，不代表 Bid Governance / Campaign Studio / Bulk 执行解锁。</div>
      `;
      const bidPreview = byId('queryNativeBidIntelligence');
      if (bidPreview?.parentElement === host) host.insertBefore(root, bidPreview.nextElementSibling || wrap);
      else host.insertBefore(root, wrap);
    }
    bindUi(root);
    render();
    return true;
  ...
  function bindUi(root) {
    ...
    byId('btnBidGovernanceParityAudit')?.addEventListener('click', () => {
      run({ force: true }).catch(error => console.warn('Bid Governance parity audit blocked/failed:', error));
    });
  }

  function render() {
    const root = byId('bidGovernanceParityAudit');
    if (!root) return;
    const banner = byId('bidGovernanceParityBanner');
    const kpis = byId('bidGovernanceParityKpis');
    const grid = byId('bidGovernanceParityGrid');
    const eligibility = state.eligibility || rawEligibility(currentRequest());

    if (state.status === 'loading') {
      banner.dataset.kind = 'warn';
      banner.textContent = '正在对同一筛选范围执行 Legacy Raw ↔ TiDB Query 双源对账…';
      kpis.innerHTML = '';
      grid.innerHTML = '';
      return;
    }
    if (state.status === 'error') {
      banner.dataset.kind = 'bad';
      banner.textContent = `对账失败：${state.lastError || 'unknown error'}`;
      kpis.innerHTML = '';
      grid.innerHTML = '';
      return;
    }
    if (state.status === 'blocked' || !eligibility.ready) {
      banner.dataset.kind = 'warn';
      banner.textContent = `只读门禁：${(eligibility.reasons || []).join('；') || '需要覆盖当前范围的 Raw 明细'}`;
      kpis.innerHTML = '';
      grid.innerHTML = '';
      return;
    }
    if (state.status !== 'ready' || !state.comparison) {
      banner.dataset.kind = 'good';
      banner.textContent = `Raw 对账前提已满足：${eligibility.loadedRange?.fromMonth || '—'} → ${eligibility.loadedRange?.toMonth || '—'}。点击“运行双源对账”。`;
      kpis.innerHTML = '';
      grid.innerHTML = '';
      return;
    }
    ...
    kpis.innerHTML = [
      ...
      ['Spend Δ', fmtMoney(c.metrics.spend.absolute)],
      ['Sales Δ', fmtMoney(c.metrics.sales.absolute)],
      ['Orders Δ', fmtSigned(c.metrics.orders.absolute, 0)],
      ['Group overlap', fmtPct(c.groupOverlap)],
      ['Bid mismatch', `${fmtInt(c.bidMismatch)} + ${fmtInt(c.bidMissingEither)} missing`],
    ].map(([label, value]) => `<div class="bgpaKpi"><span>${escapeHtml(label)}</span><b>${escapeHtml(value)}</b></div>`).join('');

    const metricRows = [
      ['Rows', c.rowCount, value => fmtInt(value)],
      ['Impressions', c.metrics.impressions, value => fmtInt(value)],
      ['Clicks', c.metrics.clicks, value => fmtInt(value)],
      ['Spend', c.metrics.spend, value => fmtMoney(value)],
      ['Sales', c.metrics.sales, value => fmtMoney(value)],
      ['Orders', c.metrics.orders, value => fmtInt(value)],
    ];
    const metricTable = `
      <div class="bgpaTableWrap"><table class="bgpaTable"><thead><tr><th>Metric</th><th>Legacy</th><th>Query</th><th>Δ</th><th>Rel</th><th>State</th></tr></thead><tbody>
        ${metricRows.map(([name, item, formatter]) => `<tr><td>${escapeHtml(name)}</td><td class="num">${escapeHtml(formatter(item.legacy))}</td><td class="num">${escapeHtml(formatter(item.query))}</td><td class="num">${escapeHtml(name === 'Spend' || name === 'Sales' ? fmtMoney(item.absolute) : fmtSigned(item.absolute, 0))}</td><td class="num">${escapeHtml(fmtPct(item.relative))}</td><td><span class="bgpaTag ${item.pass ? 'good' : 'bad'}">${item.pass ? 'PASS' : 'GAP'}</span></td></tr>`).join('')}
      </tbody></table></div>`;

    const mismatchTable = `
      <div class="bgpaTableWrap"><table class="bgpaTable"><thead><tr><th>Object</th><th>Side</th><th>Spend L/Q</th><th>Sales L/Q</th><th>Orders L/Q</th><th>Bid L/Q</th></tr></thead><tbody>
        ${(c.mismatches || []).length ? c.mismatches.map(row => `<tr><td title="${escapeHtml([row.campaign,row.adGroup,row.targeting].filter(Boolean).join(' / '))}">${escapeHtml(row.targetingId || row.targeting || row.key)}</td><td>${escapeHtml(row.side || row.bidState || 'metric')}</td><td class="num">${escapeHtml(`${fmtMoney(row.legacySpend)} / ${fmtMoney(row.querySpend)}`)}</td><td class="num">${escapeHtml(`${fmtMoney(row.legacySales)} / ${fmtMoney(row.querySales)}`)}</td><td class="num">${escapeHtml(`${fmtInt(row.legacyOrders)} / ${fmtInt(row.queryOrders)}`)}</td><td class="num">${escapeHtml(`${row.legacyBid ?? '—'} / ${row.queryBid ?? '—'}`)}</td></tr>`).join('') : '<tr><td colspan="6">当前阈值下没有对象级差异。</td></tr>'}
      </tbody></table></div>`;
  ...
  function markStale(reason = '筛选条件已变化') {
    ...
    setState({ status: 'stale', lastError: reason });
    const banner = byId('bidGovernanceParityBanner');
    if (banner) {
      banner.dataset.kind = 'warn';
      banner.textContent = `${reason}；请重新运行双源对账。`;
    }
  }

  function bindScopeWatchers() {
    if (window.__BID_GOV_PARITY_WATCHERS_BOUND__) return;
    window.__BID_GOV_PARITY_WATCHERS_BOUND__ = true;
    document.addEventListener('change', event => {
      if (FILTER_IDS.includes(event.target?.id)) markStale('筛选条件已变化');
    }, true);
    document.addEventListener('input', event => {
      if (event.target?.id === 'filterSearchTerm') markStale('搜索条件已变化');
    }, true);
  ...
  function auditError(status, message) {
    const error = new Error(message);
    error.status = status;
    return error;
  }

  window.BidGovernanceParityAudit = Object.freeze({
    version: AUDIT_VERSION,
    run,
    compareRows,
    summarizeRows,
  ...
  const init = () => {
    ensureUi();
    bindScopeWatchers();
    dispatch('lr:bid-governance-parity-audit-ready', {
      version: AUDIT_VERSION,
      executionAuthorized: false,
    });
  };
